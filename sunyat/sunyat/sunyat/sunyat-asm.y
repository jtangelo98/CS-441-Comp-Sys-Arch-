%include {
	...
	unsigned char msg_data [APP_ID_SIZE];
	unsigned char rom_data [APP_ROM_SIZE];
	...
	void store_instruction () {
		rom_data [address++] = (high_opcode << 3) | (high_reg);
	...
	if (errors_found == 0) {
		if (assembler_pass == 2) {
			...
			if (outfile == NULL) {
				...
				fwrite (msg_data, sizeof (char), APP_ID_SIZE, outfile);
				fwrite (rom_data, sizeof (char), APP_ROM_SIZE, outfile);
				fclose (outfile);
			}
		}
	}
...
message_line ::= MESSAGE(msg).	{
	if (assembler_pass == 1) {
		...
		else if (strlen (msg.token_str) > APP_ID_SIZE) {
			char s[100];
			sprintf (s, ".MESSAGE string longer than %d characters.", APP_ID_SIZE);
			warning (msg, s, NULL);
			memcpy (msg_data, msg.token_str, APP_ID_SIZE);
		}
		else {
			int i;
			memcpy (msg_data, msg.token_str, strlen (msg.token_str));
			for (i = strlen (msg.token_str); i < APP_ID_SIZE; i++) {
				msg_data [i] = '\0';
			}
		}
	}
...
address_line ::= ADDR_DIRECTIVE immediate(immed).	{
	if ((unsigned char)immed.data >= APP_ROM_SIZE) {
		if (assembler_pass == 1) {
